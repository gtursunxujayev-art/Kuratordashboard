              ...
              {!students || students.length === 0 ? (
                ...
                students.map((student) => (
                  ...
                  >
                    ...
                    {student.phone && <div className="text-xs text-gray-400 mt-0.5">{student.phone}</div>}
                  </button>
                ))
              )}
            </div>
              ...
              <div className="p-4">
                {!exerciseData ? (
                  ...
                  <div className="text-center text-gray-400 text-sm py-6">
                    {exerciseData.courseRunId ? 'Bu rejim uchun mashqlar topilmadi' : "Faol oqim yo'q"}
                  </div>
                ) : (
                      ...
                      <div className="p-3 bg-blue-50 rounded-lg border border-blue-100 space-y-2">
                        ...
                        <p className="text-xs text-blue-600">
                          ...
                          {exerciseData.attendanceSummary.isPremiumEligible
                            ? ` | Premium/VIP: ${exerciseData.attendanceSummary.premiumExtra.attended}/${exerciseData.attendanceSummary.premiumExtra.total}`
                            : ''}
                        </p>
                        {exerciseData.isClassDay && dateMode !== 'all' && (
                    ...
                    {exerciseData.exercises.map((exercise) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={
                            dateMode === 'all' || (logMutation.isLoading && loggingExerciseId === exercise.id)
                          }
                          className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-colors ${
                            exercise.doneTotal >= exercise.targetCount
                        ...
                        >
                          {dateMode === 'all'
                            ? 'Faqat ko\'rish'
                            : logMutation.isLoading && loggingExerciseId === exercise.id
                            ? '...'
                            : exercise.doneTotal >= exercise.targetCount
                            ? 'Bajarildi'
                            : "Qo'shish"}
